...
class FilterBoxSingleViz(BaseViz):

    """Lynx Analytics single-select filter box"""

    viz_type = "filter_box_single"
    verbose_name = _("Filters Single")
    is_timeseries = False
    credits = 'a <a href="https://github.com/airbnb/superset">Superset</a> original'
    fieldsets = ({
        'label': None,
    ...
    def query_obj(self):
        qry = super(FilterBoxSingleViz, self).query_obj()
        groupby = self.form_data.get('groupby')
        if len(groupby) < 1 and not self.form_data.get('date_filter'):
            raise Exception("Pick at least one filter field")
        qry['metrics'] = [
    ...
    def get_data(self):
        ...
        for flt in filters:
            qry['groupby'] = [flt]
            df = super(FilterBoxSingleViz, self).get_df(qry)
            d[flt] = [{
                'id': row[0],
                'text': row[0],
                'filter': flt,
...
class DateFilterSliderViz(BaseViz):

    """Lynx Analytics date filter slider"""

    viz_type = "date_filter_slider"
    verbose_name = _("Date Filter Slider")
    is_timeseries = False
    ...
    def query_obj(self):
        qry = super(DateFilterSliderViz, self).query_obj()
        groupby = self.form_data.get('groupby')
        if len(groupby) < 1 and not self.form_data.get('date_filter'):
            raise Exception("Pick at least one filter field")
        qry['metrics'] = [
    ...
    def get_data(self):
        ...
        for flt in filters:
            qry['groupby'] = [flt]
            df = super(DateFilterSliderViz, self).get_df(qry)
            d[flt] = [{
                'id': row[0],
                'text': row[0],
                'filter': flt,
